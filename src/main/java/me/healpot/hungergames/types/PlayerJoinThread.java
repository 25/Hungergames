...
import me.healpot.hungergames.configs.LoggerConfig;
...
import me.healpot.hungergames.managers.ReflectionManager;
import me.healpot.hungergames.utilities.ProfileUtils;
            ...
            if (HungergamesApi.getConfigManager().getMySqlConfig().isMysqlKitsEnabled()) {
                ...
                if (!tables.next()) {
                    ...
                    if (manager.hasGameProfiles() && !tables.next()) {
                        tables.close();
                        System.out.println("[Hungergames] Updating mysql to support UUID's");
                        Statement stmt = con.createStatement();
                        ...
                        for (String name : namesToConvert) {
                            ProfileUtils.PlayerProfile profile = ProfileUtils.lookup(name);
                            UUID uuid = profile.getId();
                            if (uuid != null) {
                                stmt.execute("UPDATE `HGKits` SET uuid = '" + uuid.toString() + "' WHERE Name='" + name + "'");
                                System.out.print("[Hungergames] Converted " + name + " and added uuid " + uuid.toString());
                            } else {
                                System.out.print("[Hungergames] Failed to find a UUID for " + name);
                            }
                        }
                        stmt.close();
                    } else {
                    ...
                    if (!tables.next()) {
                        tables.close();
                        System.out.println("[Hungergames] Updating mysql to support timestamps for HGKits");
                        Statement stmt = con.createStatement();
