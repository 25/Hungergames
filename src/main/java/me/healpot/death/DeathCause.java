...
import me.healpot.death.causes.*;
...
public abstract class DeathCause {
    ...
    static {
        ...
        deathCauses.add(DeathCause.FIREBALL);
    ...
    public static void registerDeathCause(DeathCause cause, boolean beforeLast) {
        if (beforeLast) {
            deathCauses.add(deathCauses.size() - 1, cause);
        } else {
    ...
    public static String getEntityName(Object obj) {
        ...
        if (obj instanceof Entity) {
            ...
            if (entity instanceof Wolf && ((Wolf) entity).isTamed()) {
                return ((Wolf) entity).getOwner().getName() + "'s wolf";
            }
    ...
    public abstract Entity getKiller(EntityDamageEvent event);
